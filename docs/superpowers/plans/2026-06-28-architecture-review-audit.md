...
- **Interfaces (produced):**
  ...
  - `rank(findings: list[dict], blast_fn, priors: dict[str, float] = AXIS_STAKES_PRIOR) -> list[dict]` — returns a **permutation** of the input. Order: unconfirmed tier (grade ≠ `deterministic_single`/`mechanical_fact`... see Constraints) first, then within each tier by `priors[axis] * finding_blast` descending.
...
- **Constraints (CRITICAL):**
  ...
  - **Tier split:** "confident" = a finding that has passed the §10 gate (carry a boolean the caller sets, e.g. `finding["confident"]`); confident findings sort **below** unconfirmed. Do not re-derive confidence from grade here — the gate (Phase 2) sets it.
...
- **Risks:** Conflating "confident" with grade; the gate, not the grade, decides the tier.
...
- **Tests (pinned assertions):**
  ...
  - *Tier precedence:* an unconfirmed finding sorts before a confident finding even when the confident one has higher `prior × blast`.
...
- **Acceptance:** dispatching it over the spike fixture (`scratchpad/fixture/app`) reproduces the bets-2+3 behavior: detects the planted divergences, stays neutral on the prevalence-trap, null recommendations on anchorless forks. (Manual/dogfood acceptance — not a unit test.)
- **Risks:** Prompt drift from the taxonomy module → guarded by Task 4.1 conformance.

### Task 2.2 — SKILL.md: orchestration, output contract, human loop
...
- **Interfaces (produced):** documented orchestration steps — (1) load Tier-3 baseline + cross-run state; (2) dispatch the auditor (Task 2.1) per scope; (3) merge into state (`merge_findings`); (4) **rank** (Task 1.1) showing the prior + offering reorder; (5) human loop per the four output-contract states; (6) codify (Phase 3). Must state the §11 ranking line is **shown and overridable**, ordering-only.
- **Constraints:** Embed the `<!-- architecture-review-axes: ... -->` conformance anchor listing the 7 axes (Task 4.1 binds to it). Carry the honest-expectation + hard scope caveat from spec §2 (A does convention divergence, NOT duplication — that's #495). State the §10 confident-path gate (re-run-to-promote) and that grade for mechanically-determinable conventions comes from extraction, not an LLM label (§8).
...
### Task 3.1 — Confident-path cross-derivation guard (REQUIRED — spec §9)

- **Goal:** Turn #494's lesson into a mechanism: a reproducibly-wrong Tier-1 fact must not pass the gate as `deterministic_single`.
...
- **Interfaces (produced):** for every **confident-path** finding, corroborate its underlying Tier-1 fact against a **second derivation** (the targeted Read the auditor already did, or the LSP); on divergence, **downgrade the finding and flag `possible_extractor_bug`** (surface, do not confirm).
...
## Phase 4 — Conformance + dogfood validation
...
- **Goal:** Run the end-to-end skill over the planted-stakes fixture (and pyeye itself as tidy control) and record the **value metric**, not raw yield.
