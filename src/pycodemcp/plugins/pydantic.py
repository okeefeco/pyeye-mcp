...
class PydanticPlugin(AnalyzerPlugin):
    ...
    def get_model_schema(self, model_name: str) -> Optional[Dict[str, Any]]:
        ...
        for item in node.body:
            if isinstance(item, ast.AnnAssign) and isinstance(item.target, ast.Name):
                ...
                if item.value and isinstance(item.value, ast.Call):
                    ...
                    field_info["default"] = ast.unparse(item.value) if hasattr(ast, 'unparse') else str(item.value)
    ...
    def _is_validator_decorator(self, decorator) -> bool:
        """Check if decorator is a Pydantic validator."""
        if isinstance(decorator, ast.Name):
            return decorator.id in ["validator", "field_validator", "root_validator", "model_validator"]
        if isinstance(decorator, ast.Attribute):
            return decorator.attr in ["validator", "field_validator", "root_validator", "model_validator"]
        if isinstance(decorator, ast.Call):
            return self._is_validator_decorator(decorator.func)
        return False
        
    def _get_validator_type(self, decorator) -> str:
        """Get the type of validator from decorator."""
        if isinstance(decorator, ast.Name):
            return decorator.id
        if isinstance(decorator, ast.Attribute):
            return decorator.attr
        if isinstance(decorator, ast.Call) and isinstance(decorator.func, (ast.Name, ast.Attribute)):
            return self._get_validator_type(decorator.func)
        return "unknown"
        
    def _get_validator_fields(self, decorator) -> List[str]:
    ...
    def _get_validator_mode(self, decorator) -> str:
            ...
            if isinstance(item, ast.Assign):
                for target in item.targets:
                    if isinstance(target, ast.Name):
                        settings[target.id] = ast.unparse(item.value) if hasattr(ast, 'unparse') else str(item.value)
                        
        return settings
        
    def _extract_config_dict(self, node) -> Dict[str, Any]:
        ...
        if isinstance(node, ast.Dict):
            for key, value in zip(node.keys, node.values):
                if isinstance(key, ast.Constant):
                    settings[key.value] = ast.unparse(value) if hasattr(ast, 'unparse') else str(value)
                    
        return settings
        
    def _is_computed_field_decorator(self, decorator) -> bool:
