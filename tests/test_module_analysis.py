...
class TestListPackages:
    ...
    def test_list_packages_empty_project(self, tmp_path):
        """Test listing packages in an empty project."""
        result = list_packages(str(tmp_path))
        assert result == []

    def test_list_packages_single_package(self, tmp_path):
    ...
    def test_list_packages_nested(self, tmp_path):
    ...
    def test_list_packages_ignores_hidden(self, tmp_path):
    ...
    def test_list_packages_project_not_found(self):
        ...
        with pytest.raises(FileAccessError) as exc_info:
            list_packages("/non/existent/path")
...
class TestListModules:
    ...
    def test_list_modules_empty_project(self, tmp_path):
        """Test listing modules in an empty project."""
        result = list_modules(str(tmp_path))
        assert result == []

    def test_list_modules_single_file(self, tmp_path):
...
'''
    ...
    def test_list_modules_with_imports(self, tmp_path):
...
"""
    ...
    def test_list_modules_in_package(self, tmp_path):
    ...
    def test_list_modules_skips_tests(self, tmp_path):
...
class TestAnalyzeDependencies:
    ...
    def test_analyze_dependencies_simple(self, tmp_path):
...
"""
        ...
        result = analyze_dependencies("module", str(tmp_path))
    ...
    def test_analyze_dependencies_internal(self, tmp_path):
...
"""
        ...
        result = analyze_dependencies("module_b", str(tmp_path))
        ...
        result_a = analyze_dependencies("module_a", str(tmp_path))
        assert "module_b" in result_a["imported_by"]

    def test_analyze_dependencies_circular(self, tmp_path):
        """Test detecting circular dependencies."""
        # Create circular dependency
        (tmp_path / "module_a.py").write_text("import module_b")
        (tmp_path / "module_b.py").write_text("import module_a")

        result = analyze_dependencies("module_a", str(tmp_path))
        assert "module_b" in result["imports"]["internal"]
        assert "module_b" in result["circular_dependencies"]

    def test_analyze_dependencies_module_not_found(self, tmp_path):
        """Test error when module doesn't exist."""
        with pytest.raises(FileAccessError) as exc_info:
            analyze_dependencies("nonexistent", str(tmp_path))
        assert "Module not found" in str(exc_info.value)

    def test_analyze_dependencies_package(self, tmp_path):
...
"""
        ...
        result = analyze_dependencies("mypackage.module", str(tmp_path))
...
class TestGetModuleInfo:
    ...
    def test_get_module_info_simple(self, tmp_path):
...
'''
        ...
        result = get_module_info("module", str(tmp_path))
        assert result["module"] == "module"
    ...
    def test_get_module_info_with_imports(self, tmp_path):
...
"""
    ...
    def test_get_module_info_complexity(self, tmp_path):
...
"""
    ...
    def test_get_module_info_module_not_found(self, tmp_path):
        """Test error when module doesn't exist."""
        with pytest.raises(FileAccessError) as exc_info:
            get_module_info("nonexistent", str(tmp_path))
        assert "Module not found" in str(exc_info.value)

    def test_get_module_info_includes_dependencies(self, tmp_path):
...
class TestJediAnalyzerMethods:
    ...
    def test_list_packages_method(self, tmp_path):
        ...
        analyzer = JediAnalyzer(str(tmp_path))
        result = analyzer.list_packages()
        assert len(result) == 1
        assert result[0]["name"] == "package"

    def test_list_modules_method(self, tmp_path):
        """Test list_modules method directly."""
        (tmp_path / "module.py").write_text("def func(): pass")

        analyzer = JediAnalyzer(str(tmp_path))
        result = analyzer.list_modules()
        assert len(result) == 1
        assert result[0]["name"] == "module"
    ...
    def test_analyze_dependencies_method(self, tmp_path):
        ...
        analyzer = JediAnalyzer(str(tmp_path))
        result = analyzer.analyze_dependencies("module")
        assert "os" in result["imports"]["stdlib"]

    def test_get_module_info_method(self, tmp_path):
        """Test get_module_info method directly."""
        (tmp_path / "module.py").write_text('"""Doc."""\ndef func(): pass')

        analyzer = JediAnalyzer(str(tmp_path))
        result = analyzer.get_module_info("module")
